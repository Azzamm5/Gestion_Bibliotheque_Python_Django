...
def request_form(request):
	...
	return render(request, 'publisher/delete_request.html',context)
...
def delete_request(request):
	if request.method == 'POST':
		...
		user_request = username + "  veut emprunter le livre avec l'id  " + book_id
		current_time = timezone.now()

		a = DeleteRequest(delete_request=user_request, timestamp=current_time)
		a.save()
		messages.success(request, 'Message Envoyer')
		return redirect('request_form')
	else:
		messages.error(request, 'Message Non Envoyer')
	return redirect('request_form')
...
def send_feedback(request):
	if request.method == 'POST':
		...
		user_id = current_user.id
		username = current_user.username
		feedback = username + " " + " a signaler que  " + feedback
		current_time = timezone.now()

		a = Feedback(feedback=feedback, timestamp=current_time)
		a.save()
...
def dashboard(request):
	book = Book.objects.all().count()
	user = User.objects.all().count()
	message = Chat.objects.all().count()

	context = {'book':book, 'user':user,  'chats':message}
